...
const Admin = ({ products, onProductsChange, orders, offers, onOffersChange, settings, onSettingsChange, notifications, onNotificationsChange }: AdminProps) => {
  ...
  const addProduct = () => {
    ...
    onProductsChange([...products, { id: nextId, name: newProduct.name, nameAr: newProduct.name, price: Number.isNaN(price) ? 0 : price, originalPrice: Number.isNaN(originalPrice) ? null : originalPrice, rating: 4.5, image: newProduct.image, category: newProduct.category, unit: newProduct.unit, inStock: true }]);
  ...
  if (!isAuthenticated) {
    return <div className="min-h-screen bg-background flex items-center justify-center px-4" dir={language === "ar" ? "rtl" : "ltr"}><div className="w-full max-w-md bg-card rounded-3xl shadow-card p-8 space-y-5 text-center"><Button variant="outline" size="sm" className="ml-auto" onClick={() => setLanguage((prev) => (prev === "en" ? "ar" : "en"))}>{t.lang}</Button><div className="mx-auto w-20 h-20 rounded-2xl bg-primary/10 flex items-center justify-center"><img src={roboLogoDataUrl} alt="Robo Al Ain logo" className="w-16 h-16 object-contain" /></div><h1 className="text-2xl font-bold">{t.login}</h1><Input placeholder={t.user} value={username} onChange={(event) => setUsername(event.target.value)} /><Input placeholder={t.pass} type="password" value={password} onChange={(event) => setPassword(event.target.value)} />{authError ? <p className="text-sm text-destructive">{authError}</p> : null}<Button className="w-full" onClick={handleLogin}>{t.signIn}</Button></div></div>;
  }
  ...
  return (
    <div className="min-h-screen bg-background" dir={language === "ar" ? "rtl" : "ltr"}>
      ...
      <div className="container mx-auto px-4 py-6 grid lg:grid-cols-[240px_1fr] gap-6">
        <aside className="bg-card rounded-2xl p-4 shadow-card h-fit"><nav className="space-y-1">{menu.map((item) => <Link key={item.path} to={item.path} className={`block rounded-lg px-3 py-2 text-sm ${location.pathname === item.path ? "bg-primary text-primary-foreground" : "hover:bg-secondary"}`}>{item.label[language]}</Link>)}</nav></aside>
